...
import type { CampaignDatum, Milestone } from '../types/mesh';
...
export function encodeCampaignDatum(datum: CampaignDatum): EncodedDatum | CampaignDatum {
  try {
    ...
    const safeEncode = (value: unknown, defaultValue: unknown, type: string = 'any'): unknown => {
      if (value === undefined || value === null) {
        console.warn(`⚠️ Using default for undefined value: ${defaultValue}`);
        return defaultValue;
      }
      
      if (type === 'number' && (isNaN(Number(value)) || !isFinite(Number(value)))) {
    ...
    const campaignDatum: EncodedDatum = {
...
export function encodeRedeemer(campaignId: number | undefined, action: string, actionData: ActionData = {}): EncodedRedeemer {
  try {
    console.log('🔧 Encoding redeemer for Aiken validator...');
    
    // Map action strings to your Aiken MedicalAction constructors
    const getActionConstructor = (actionName: string): { constructor: number; fields: unknown[] } => {
      switch (actionName) {
        case "CreateCampaign":
    ...
    const redeemer: EncodedRedeemer = {
...
export function decodeCampaignDatum(plutusData: unknown): CampaignDatum | null {
  try {
    // Simplified decoder - in production, you'd use proper CBOR decoding
    if (typeof plutusData === 'string') {
      return JSON.parse(plutusData) as CampaignDatum;
    }
    
    // Handle MeshJS datum structure
    if (plutusData && typeof plutusData === 'object' && 'constructor' in plutusData) {
      return plutusData as CampaignDatum;
    }
...
export function getCampaignStatus(statusConstructor: number | { constructor: number; fields: unknown[] } | undefined): string {
  ...
  if (typeof statusConstructor === 'object' && statusConstructor && 'constructor' in statusConstructor) {
    return statusMap[statusConstructor.constructor] || 'Unknown';
  }
  
  if (typeof statusConstructor === 'number') {
...
export function getCampaignStatusText(statusConstructor: number | { constructor: number; fields: unknown[] } | undefined): string {
  return getCampaignStatus(statusConstructor);
}

// Get campaign status with color coding for UI
export function getCampaignStatusWithColor(statusConstructor: number | { constructor: number; fields: unknown[] } | undefined): StatusColor {
...
export function getCampaignProgress(campaign: CampaignDatum | null | undefined): CampaignProgress {
...
export function getCampaignTimeProgress(campaign: CampaignDatum | null | undefined): TimeProgress {
...
export function getTimeRemaining(campaign: CampaignDatum | null | undefined): TimeRemaining {
  if (!campaign || !campaign.deadline) {
    return {
      days: 0,
      hours: 0,
...
export function canRequestRefund(campaign: CampaignDatum | null | undefined): boolean {
  ...
  return (
    status === 'Cancelled' ||
    (status === 'Active' && now > deadline && (campaign.current_funds || 0) < (campaign.total_goal || 0)) ||
    status === 'Paused'
  );
}

// Milestone helpers
export function getNextMilestone(campaign: CampaignDatum | null | undefined): Milestone | null {
  if (!campaign || !campaign.milestones) return null;
  
  return campaign.milestones.find(milestone => !milestone.claimed) || null;
}

export function getMilestoneProgress(campaign: CampaignDatum | null | undefined): MilestoneProgress {
  if (!campaign || !campaign.milestones || campaign.milestones.length === 0) {
    return { completed: 0, total: 0, percentage: 0 };
  }
  
  const completed = campaign.milestones.filter((m: Milestone) => m.claimed).length;
...
export function canClaimMilestoneHelper(campaign: CampaignDatum | null | undefined, milestonePercentage: number, currentTime: number = Date.now()): boolean {
  ...
  const currentFunds = campaign.current_funds || 0;
  const fundingPercentage = totalGoal > 0 ? (currentFunds / totalGoal) * 100 : 0;
...
export function getLastMilestoneClaimTime(milestones: Milestone[] | undefined): number {
  if (!milestones || milestones.length === 0) return 0;
  
  // Find the most recent claim date from claimed milestones
  const claimedMilestones = milestones.filter(m => m.claimed && (m.claim_date || 0) > 0);
  
  if (claimedMilestones.length === 0) return 0;
  
  return Math.max(...claimedMilestones.map((m: Milestone) => m.claim_date || 0));
}

// Calculate claimable amount for a milestone
export function getClaimableAmount(campaign: CampaignDatum | null | undefined, milestonePercentage: number): number {
  if (!campaign) return 0;
  
  const totalGoal = campaign.total_goal || 0;
  const currentFunds = campaign.current_funds || 0;
  const totalClaimed = campaign.total_claimed || 0;
  const milestoneAmount = Math.floor((totalGoal * milestonePercentage) / 100);
  return Math.min(milestoneAmount, currentFunds - totalClaimed);
}

// Get all claimable milestones
export function getClaimableMilestones(campaign: CampaignDatum | null | undefined, currentTime: number = Date.now()): Milestone[] {
  if (!campaign || !campaign.milestones) return [];
  
  return campaign.milestones.filter((milestone: Milestone) => 
    canClaimMilestoneHelper(campaign, milestone.percentage, currentTime)
  );
}

// Get the next claimable milestone (single milestone)
export function getNextClaimableMilestone(campaign: CampaignDatum | null | undefined, currentTime: number = Date.now()): Milestone | null {
  if (!campaign || !campaign.milestones) return null;
  ...
  return claimableMilestones.reduce((next: Milestone, current: Milestone) => 
    current.percentage < next.percentage ? current : next
  );
}

// Address validation helpers
export function isValidCardanoAddress(address: string | null | undefined): boolean {
...
export function formatAddress(address: string | null | undefined, length: number = 12): string {
  if (!address) return '';
