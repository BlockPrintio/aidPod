 ...
 */
export async function buildSimpleClaimTx(wallet, campaignData, milestonePercentage) {
  try {
    ...
    const totalGoalLovelace = campaignData.goalAda * 1000000; // Convert ADA to lovelace
    const currentFundsLovelace = campaignData.currentAmount * 1000000;
    ...
    const unsignedTx = await txBuilder
      .txOut(campaignData.beneficiary, [
    ...
    throw new Error(`Claim transaction failed: ${error.message}`);
  }
}
 ...
 */
export function canClaimMilestone(campaignData, milestonePercentage, userAddress) {
  ...
  const totalGoalLovelace = campaignData.targetAmount * 1000000;
  const currentFundsLovelace = campaignData.currentAmount * 1000000;
  const milestoneThreshold = Math.floor(totalGoalLovelace * milestonePercentage / 100);
  const claimableAmount = Math.min(currentFundsLovelace, milestoneThreshold);
 ...
 */
export function getAvailableMilestones(campaignData, userAddress) {
