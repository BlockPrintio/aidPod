...
export async function buildCreateCampaignTx(wallet, campaignData) {
  try {
    ...
    try {
      ...
      console.error('❌ Error getting UTxOs:', utxoError);
      throw new Error(`Failed to get UTxOs: ${utxoError.message}`);
    }
    ...
    try {
      ...
      console.error('❌ Error getting change address:', addressError);
      throw new Error(`Failed to get change address: ${addressError.message}`);
    }
    ...
    const encodedRedeemer = encodeRedeemer(campaignDatum.campaign_id, "CreateCampaign");
    ...
    console.error('Error building create campaign transaction:', error);
