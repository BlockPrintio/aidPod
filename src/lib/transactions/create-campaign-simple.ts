 ...
 */
export async function buildSimpleCampaignTx(wallet: MeshWallet, campaignData: CampaignData): Promise<string> {
  try {
    ...
    console.error('❌ Error building simple transaction:', err);
    throw new Error(`Simple transaction failed: ${err.message}`);
  }
}
 ...
 */
export async function testWalletConnection(wallet: MeshWallet): Promise<WalletTestResult> {
  try {
